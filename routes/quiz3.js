...
router.get("/getCount", (req, res) => {
 ...
 //   var upperLimit = req.query.lat_second;
    var sql = `Select year,sum(NumberTerroristIncidents) as count from ti where Code = '${lowerLimit}' group by year`;
...
router.post("/getQuakes1", (req, res) => {
    ...
    var start  = Date.now();
    var sql  = `SELECT Smokers,year from s
   ...
   // var sql = `select * from sp where Entity in ('Afghanistan','Bahamas','Canada','Egypt','France') and year between ${lowerLimit} and ${upperLimit} order by 1,3;`
    const request = new Request(sql,
        (err, rowCount, rows) => {
            if (err) {
                console.error(err.message);
...
router.get("/get2FromRedis", (req,res) => {
    ...
    var range = Number(req.query.range);
    var valn = Number(req.query.lat_first)
    var valyear =  Number(req.query.lat_second);
    var result = [];
    var totalpop;

   // exesql(val1,val1+0.1,range,0);
  //  var totalstartTime = Date.now();
    var sql = `select state,"${valyear}" as popl from pop where "${valyear}" = (select max("${valyear}") from pop)`; 
    const request = new Request(sql,
        (err, rowCount, rows) => {
            if (err) {
                console.error(err.message);
                ...
                var obj ={};
                console.log(rows);
                totalpop = rows[0].popl.value;
                var range = totalpop/valn;
                var val1 = 0;
                var val2 = val1 + range;
                exesql(val1,val2,range,0);
               // obj["dataRows"] = rows;
       ...
       function exesql(val1,val2,range,i) {
        ...
        // }
        var label = lowerLimit + " - " + upperLimit;
        var sql = `Select count(state) as count from pop where ${valyear} between ${lowerLimit} and ${upperLimit}`;
        if(i==totalpop ){
